Public Class ClassCustomer
    ...
    Sub Restore()
        If Not dbOpen() Then
            Throw New Exception("Database Can't Open")
        End If
        SqlCMD = New SqlClient.SqlCommand With {.CommandType = CommandType.StoredProcedure, .Connection = sqlConn, .CommandText = "Customer_Res"}
        With SqlCMD.Parameters
            .Clear()
            .AddWithValue("id", id)
        End With
    ...
    Sub Load2()
        ...
        DataCustomer.Dispose()
        DataCustomer = New DataSet
        SqlCMD = New SqlClient.SqlCommand With {.CommandType = CommandType.StoredProcedure, .Connection = sqlConn, .CommandText = "Customer_Get2"}
        With SqlCMD.Parameters
            .Clear()
