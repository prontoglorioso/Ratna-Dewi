Public Class ClassVendor
    ...
    Sub Load2()
        ...
        DataVendor.Dispose()
        DataVendor = New DataSet
        SqlCMD = New SqlClient.SqlCommand With {.CommandType = CommandType.StoredProcedure, .Connection = sqlConn, .CommandText = "Vendor_Get2"}
        With SqlCMD.Parameters
            .Clear()
    ...
    Sub Restore()
        If Not dbOpen() Then
            Throw New Exception("Database Can't Open")
        End If
        SqlCMD = New SqlClient.SqlCommand With {.CommandType = CommandType.StoredProcedure, .Connection = sqlConn, .CommandText = "Vendor_Res"}
        With SqlCMD.Parameters
            .Clear()
            .AddWithValue("id", id)
        End With
