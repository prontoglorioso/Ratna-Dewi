Public Class ClassProject
    ...
    Sub Restore()
        If Not dbOpen() Then
            Throw New Exception("Database Can't Open")
        End If
        SqlCMD = New SqlClient.SqlCommand With {.CommandType = CommandType.StoredProcedure, .Connection = sqlConn, .CommandText = "Project_Res"}
        With SqlCMD.Parameters
            .Clear()
            .AddWithValue("id", id)
        End With
    ...
    Sub Load2()
        ...
        DataProject.Dispose()
        DataProject = New DataSet
        SqlCMD = New SqlClient.SqlCommand With {.CommandType = CommandType.StoredProcedure, .Connection = sqlConn, .CommandText = "Project_Get2"}
        With SqlCMD.Parameters
            .Clear()
